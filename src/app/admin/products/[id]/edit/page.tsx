...
export default function EditProductPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/admin/products/${params.id}`).then((r) => r.ok ? r.json() : null),
    ]).then(([cats, p]) => {
      setCategories(cats);
      if (p) {
        setForm({
          name: p.name, slug: p.slug, categoryId: p.categoryId, description: p.description || "",
          price: String(p.price), originalPrice: p.originalPrice ? String(p.originalPrice) : "", image: p.image || "",
          galleryUrls: (p.galleryUrls || []).join("\n"),
          videoUrl: p.videoUrl || "", videoFile: p.videoFile || "", sku: p.sku || "",
          stock: p.stock ? String(p.stock) : "", minOrder: p.minOrder ? String(p.minOrder) : "",
  ...
  return (
    <div className="max-w-3xl">
      ...
      <h1 className="text-xl font-bold text-primary mb-6">Edit Produk</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input value={form.sku} onChange={(e) => setForm({ ...form, sku: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-accent" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="number" min="1" value={form.minOrder} onChange={(e) => setForm({ ...form, minOrder: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-accent" />
          </div>
        </div>
        ...
        <div>
          ...
          <textarea rows={3} value={form.galleryUrls} onChange={(e) => setForm({ ...form, galleryUrls: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-accent resize-none" />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="text-xs font-semibold text-muted block mb-1">Video File URL</label>
        ...
        <div>
          ...
          {variants.map((v, i) => (
            <div key={i} className="flex gap-2 mb-2 items-end">
              <div className="flex-1">
                <input value={v.name} onChange={(e) => updateVariant(i, "name", e.target.value)} placeholder="Nama" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-xs outline-none focus:border-accent" />
              </div>
              <div className="flex-1">
                <input value={v.value} onChange={(e) => updateVariant(i, "value", e.target.value)} placeholder="Value" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-xs outline-none focus:border-accent" />
              </div>
              <div className="w-20">
