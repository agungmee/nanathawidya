...
export default function EditBannerPage() {
  ...
  const [form, setForm] = useState({ title: "", subtitle: "", image: "", url: "", isActive: true });

  useEffect(() => {
    fetch("/api/admin/banners")
      .then((r) => r.ok ? r.json() : [])
      .then((banners) => {
        const b = banners.find((b: { id: string }) => b.id === params.id);
        if (b) {
          setForm({ title: b.title || "", subtitle: b.subtitle || "", image: b.image || "", url: b.url || "", isActive: b.isActive });
        }
  ...
  return (
    <div className="max-w-2xl">
      ...
      <h1 className="text-xl font-bold text-primary mb-6">Edit Banner</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
        ...
        <div>
          ...
          <input required value={form.image} onChange={(e) => setForm({ ...form, image: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-accent" />
        </div>
        <div>
          <label className="text-xs font-semibold text-muted block mb-1">Link URL (opsional)</label>
          <input value={form.url} onChange={(e) => setForm({ ...form, url: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-accent" />
        </div>
